...
(function () {
	...
	L.OWM.temperature = function (options) {
		...
		if (layer.options.legendImagePath == null) {
			layer.options.legendImagePath = 'css/images/tt4.png';
		}
		return layer;
	};
...
L.OWM.LegendControl = L.Control.extend({
	...
	initialize: function(options) {
		L.Util.setOptions(this, options);
		this._container = L.DomUtil.create('div', 'owm-legend-container leaflet-control');
	...
	_redrawLegend: function() {
		...
		for (var idx in this._legendContainer) {
			...
			item.innerHTML = '<img src="' + imgPath + '"  />';
		}
	}
});
